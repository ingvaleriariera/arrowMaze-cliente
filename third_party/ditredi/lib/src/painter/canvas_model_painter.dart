...
class CanvasModelPainter extends CustomPainter implements PaintViewPort {
  ...
  CanvasModelPainter(
    ...
    // PATCH (local fork — see arrowMaze-cliente/third_party/ditredi): the
    // original code here called `_controller.addListener(() { _isDirty =
    // true; })`. `CustomPainter` (the actual superclass — not
    // `ChangeNotifier`) has no `dispose()` hook for Flutter to call, so
    // that listener closure was never removable and stayed permanently
    // registered on the long-lived `DiTreDiController` for the life of
    // every `CanvasModelPainter` ever constructed — up to thousands per
    // play session, since `DiTreDi` (a StatelessWidget) builds a brand new
    // one on every single rebuild. Each survivor kept its full `_figures`
    // list and vertex/color buffers alive forever, which is what was
    // driving the app to an Out of Memory crash after enough elapsed
    // animation frames.
    //
    // The listener was also functionally redundant: `super(repaint:
    // _controller)` already makes Flutter repaint automatically whenever
    // `_controller` fires, bypassing `shouldRepaint`/`_isDirty` entirely
    // (see RenderCustomPaint._didUpdatePainter in the Flutter SDK). And
    // since every fresh instance already starts with `_isDirty = true`
    // (see the field default above) and DiTreDi always constructs a fresh
    // instance rather than reusing one across frames, `shouldRepaint`
    // would have returned true on this codebase's actual usage pattern
    // with or without this listener ever firing. Removed rather than
    // patched with a dispose() override, since there is no such override
    // point to hook into.
